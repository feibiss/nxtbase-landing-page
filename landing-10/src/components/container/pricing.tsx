  ...
  return (
    <Container className="py-20 mb-12" id="pricing">
      <div className="px-4 md:px-24">
        <Typography className="text-left w-full pb-5" variant={"h3"}>
          Simple and transparent pricing
        </Typography>
        <Typography variant="p" className="md:w-[100%] lg:w-[50%] w-fit pb-10 font-light">
          Great, now that we have your attention, we will actually talk about
          how we help you do the above. 
        </Typography>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          
          <PricingCard title="Freelancer" price="$699" desc="Suitable for solo-preneurs, and freelancers"/>
          <PricingCard title="Startup" price="$899" desc="Suitable for small startups"/>
          <PricingCard title="Enterprise" price="$1099" desc="Suitable for organizations" />
        </div>
      </div>
    </Container>
  );
...
const PricingCard = ({ title, price, desc }: { title: string; price: string; desc: string;}) => {
  return (
    <div className="rounded-3xl p-6 md:p-8 bg-white border border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-300 transform hover:-translate-y-1 cursor-pointer h-full flex flex-col justify-between">
      <div>
        ...
        <p className="mt-4 tracking-wider text-sm font-normal">
          {desc}
        </p>
        ...
        {Array.from({ length: 5 }, (_, index) => (
            <li key={index} className="flex gap-2 text-base p-0 py-1 hover:bg-transparent items-center">
